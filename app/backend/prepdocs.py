...
async def setup_search_info(
    ...
    search_service: str, index_name: str, location: str, azure_credential: AsyncTokenCredential, search_key: Union[str, None] = None
) -> SearchInfo:
    ...
    index_name = f"{index_name}_{location}"
    return SearchInfo(
...
#    list_file_strategy: ListFileStrategy
    ...
    if datalake_storage_account:
        ...
        list_file_strategies["default"] = ADLSGen2ListFileStrategy(
        ...
        if not os.path.isdir(local_files):
            raise ValueError(f"The provided path '{local_files}' is not a valid directory.")
        logger.info("Using local files: %s", local_files)

        # Iterate over subdirectories in local_files
        for location in os.listdir(local_files):
            location_path = os.path.join(local_files, location)
            if os.path.isdir(location_path):
                logger.info("Setting up local file strategy for location: %s", location)
...
#     await strategy.run()
async def main(strategy_map: dict, setup_index: bool = True):
    tasks = []
...
if __name__ == "__main__":
    ...
    list_file_strategies = setup_list_file_strategy(
        azure_credential=azd_credential,
        # local_files=args.files,  #check-here
        local_files=args.base_path,  #check-here
    ...
    if not use_int_vectorization and use_gptvision:
        image_embeddings_service = setup_image_embeddings_service(
            azure_credential=azd_credential,
            vision_endpoint=os.getenv("AZURE_VISION_ENDPOINT"),
            search_images=use_gptvision,
    ...
    # Iterate over each location and create strategies
    strategy_map = {}
    for location, file_strategy in list_file_strategies.items():
        ...
        # Create strategy based on vectorization flag
        if use_int_vectorization:
            if not openai_embeddings_service or not isinstance(openai_embeddings_service, AzureOpenAIEmbeddingService):
                raise Exception("Integrated vectorization strategy requires an Azure OpenAI embeddings service")
            ...
            strategy = IntegratedVectorizerStrategy(
                search_info=search_info,
                list_file_strategy=file_strategy,
                blob_manager=blob_manager,
                document_action=document_action,
                embeddings=openai_embeddings_service,
        ...
        strategy_map[location] = strategy

    # Run ingestion strategies for all locations in parallel
    loop.run_until_complete(
        main(strategy_map=strategy_map, setup_index=not args.remove and not args.removeall)
    )
